...
function ClaimBlock({
  ...
  return (
    <div className="border-b border-ink-800/70 py-3 last:border-0">
      ...
      <p className={`mt-1.5 text-sm leading-relaxed ${isMissing ? 'text-ink-500' : 'text-ink-200'}`}>
        {claim.statement}
      </p>
      {refs.length > 0 ? (
        <p className="mt-1.5 flex flex-wrap gap-x-3 gap-y-1 text-2xs">
          {refs.map((r) => (
            <ExternalLink key={r.id} href={r.url} className="text-ink-500 underline decoration-ink-700 underline-offset-2 hover:text-ink-300">
              {r.publisher}
  ...
  return (
    <div className="panel flex flex-col p-4">
      <div className="flex items-start justify-between gap-3">
        <h3 className="text-sm font-semibold text-ink-50">{CAPITAL_FIT_LABEL[kind]}</h3>
        <RatingBar rating={fit.rating} />
      </div>
      <p className="mt-1 text-2xs text-ink-500">{fitDescriptor(fit.rating)}</p>

      {fit.drivers.length > 0 ? (
        <ul className="mt-3 space-y-1.5">
          {fit.drivers.map((d) => (
            <li key={d} className="flex gap-2 text-xs leading-relaxed text-ink-300">
      ...
      <div className="mt-auto pt-3">
        ...
        <p className="mt-1 text-xs leading-relaxed text-ink-400">{fit.conditions}</p>
        {kind === 'debt' ? (
          <p className="mt-2 rounded border border-ink-800 bg-ink-950/60 px-2.5 py-1.5 text-2xs leading-relaxed text-ink-500">
...
export default async function CompanyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div>
        ...
        <div className="mt-2 flex flex-wrap items-start justify-between gap-6">
          <div className="min-w-0">
            ...
            <p className="mt-1.5 text-sm text-ink-400">
              {company.sector} <span className="text-ink-700">/</span> {company.subsector}
            </p>
          ...
          <div className="rounded-lg border border-ink-800 bg-ink-900 px-5 py-3 text-right">
            <ScoreMark score={breakdown.score} size="lg" />
            <p className="label mt-1">Origination priority score</p>
            <p className="num mt-1.5 text-2xs text-ink-500">
              ...
              { label: 'Chief executive', value: company.ceo },
              {
                ...
                value:
                  company.financeLeader === NOT_DISCLOSED ? (
                    <span className="text-ink-500">{NOT_DISCLOSED}</span>
                  ) : (
      ...
      {/* Origination signal */}
      <Section title="Origination signal">
        <div className="panel px-4 py-2">
          <FieldList
            fields={[
                  ...
                  <ul className="space-y-1">
                    {company.potentialUseOfProceeds.map((u) => (
                      <li key={u} className="flex gap-2">
                        <span aria-hidden="true" className="mt-2 h-1 w-1 shrink-0 rounded-full bg-ink-600" />
                        <span>{u}</span>
                      </li>
                    ))}
                  </ul>
          ...
          <table className="w-full min-w-[60rem] border-collapse text-left">
            ...
            <tbody>
              {breakdown.contributions.map((c) => (
                <tr key={c.key} className="border-b border-ink-800/60 align-top">
                  <td className="px-3 py-3">
                    <p className="text-xs font-semibold text-ink-100">{c.label}</p>
                  ...
                  <td className="whitespace-nowrap px-3 py-3">
                    ...
                    {c.suppressed ? (
                      <p className="mt-1 text-2xs text-rose-400">
                        Suppressed to 0 by provenance
                      </p>
                    ) : null}
                  </td>
                  <td className="num px-3 py-3 text-right text-ink-400">{c.weight}%</td>
                  <td className="num px-3 py-3 text-right font-semibold text-ink-100">
                    {c.points.toFixed(1)}
                    <span className="text-ink-600">/{c.maxPoints}</span>
                  </td>
                  <td className="px-3 py-3">
                    <p className="text-xs leading-relaxed text-ink-300">{c.evidence}</p>
                    <p className="mt-1.5 text-xs leading-relaxed text-ink-500">{c.explanation}</p>
                    {c.sourceIds.length > 0 ? (
                      <p className="mt-1.5 flex flex-wrap gap-x-3 text-2xs">
                        {company.sources
                          .filter((s) => c.sourceIds.includes(s.id))
                          .map((s) => (
                            <ExternalLink key={s.id} href={s.url} className="text-ink-600 underline decoration-ink-700 underline-offset-2 hover:text-ink-400">
                              {s.publisher}
                            </ExternalLink>
                          ))}
                      </p>
              ...
              <tr className="border-b border-ink-800">
                <td className="px-3 py-3 text-xs font-semibold text-ink-100" colSpan={3}>
                  Base weighted score
                </td>
                <td className="num px-3 py-3 text-right font-semibold text-ink-50">
              ...
              <tr>
                <td className="px-3 py-3 text-sm font-semibold text-ink-50" colSpan={3}>
                  Origination priority score
                </td>
                <td className="num px-3 py-3 text-right text-base font-semibold text-accent-300">
        ...
        <div className="grid gap-3 lg:grid-cols-3">
          {[
            ...
            <div key={r.label} className="panel p-4">
              ...
              <p className="mt-2 text-xs leading-relaxed text-ink-300">{r.value}</p>
            </div>
          ))}
        </div>
      </Section>
          ...
          {company.outreach.emails.map((email) => (
            <article key={email.audience} className="panel flex flex-col p-4">
              <div className="flex items-center justify-between gap-3">
                ...
                <p className="text-2xs text-ink-600">Draft, not sent</p>
              </div>
              <p className="mt-2.5 text-xs text-ink-400">{email.recipientRole}</p>
              <p className="mt-2 text-xs font-semibold text-ink-100">
                Subject: {email.subject}
              </p>
              <pre className="mt-3 whitespace-pre-wrap break-words font-sans text-xs leading-relaxed text-ink-300">
                {email.body}
              </pre>
            </article>
          ))}
        ...
        <div className="panel mt-3 p-4">
          ...
          <ol className="mt-2 space-y-2">
            {company.qualificationQuestions.map((q, i) => (
              <li key={q} className="flex gap-2.5 text-xs leading-relaxed text-ink-300">
                <span className="num shrink-0 text-accent-400">{i + 1}</span>
                <span>{q}</span>
              </li>
            ))}
          </ol>
          <div className="mt-4 border-t border-ink-800 pt-3">
            ...
            <p className="mt-1.5 text-xs leading-relaxed text-ink-300">{company.nextDiligenceStep}</p>
          </div>
        </div>
      </Section>
        ...
        <div className="panel p-4">
          <ul className="grid gap-x-6 gap-y-2 sm:grid-cols-2 lg:grid-cols-3">
            {company.missingInformation.map((m) => (
              <li key={m} className="flex gap-2 text-xs text-ink-400">
                <span aria-hidden="true" className="mt-1.5 h-1 w-1 shrink-0 rounded-full bg-rose-800" />
                <span>
                  {m}: <span className="text-ink-500">{NOT_DISCLOSED}</span>
                </span>
              </li>
            ))}
          </ul>
          ...
          {[
            ...
            <div key={group.title} className="panel p-4">
              <h3 className="text-sm font-semibold text-ink-100">
                {group.title} <span className="num text-ink-600">({group.list.length})</span>
              </h3>
              <ul className="mt-3 space-y-3">
                {group.list.map((s) => (
                  <li key={s.id} className="border-b border-ink-800/60 pb-3 last:border-0 last:pb-0">
                    <ExternalLink href={s.url} className="text-xs font-medium text-accent-300 hover:text-accent-200">
                      {s.title}
                    </ExternalLink>
              ...
              {group.list.length === 0 ? (
                <p className="mt-3 text-xs text-ink-500">None recorded.</p>
              ) : null}
            </div>
          ))}
        </div>
