...
function ItemRow({ item }: { item: ReadinessItem }) {
  return (
    <li className="flex gap-2.5 border-b border-white/[0.06] py-2.5 last:border-0">
      <span
        aria-hidden="true"
        className={`mt-1 grid h-3.5 w-3.5 shrink-0 place-items-center rounded-full border text-[8px] font-bold ${
          item.available
            ? 'border-positive-500/50 bg-positive-700/30 text-positive-200'
            : 'border-white/12 bg-white/[0.03] text-slate-600'
        }`}
      >
        {item.available ? '✓' : '·'}
      </span>
      <div className="min-w-0">
        <p
          className={`text-xs font-medium ${item.available ? 'text-slate-200' : 'text-slate-500'}`}
        >
          {item.label}
        ...
        <p className="mt-0.5 text-2xs leading-relaxed text-slate-500">{item.detail}</p>
      </div>
    </li>
  );
}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="panel-raised p-5">
        ...
        <p className="mt-4 rounded-lg border border-white/[0.07] bg-navy-900/60 px-3.5 py-2.5 text-2xs leading-relaxed text-slate-500">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="panel p-5">
          <h3 className="text-sm font-semibold text-ivory-50">Publicly supported information</h3>
        ...
        <div className="panel p-5">
          <h3 className="text-sm font-semibold text-ivory-50">Information required from management</h3>
          <p className="mt-1.5 text-2xs text-slate-500">
          ...
          {required.length > 6 ? (
            <button
              ...
              className={`mt-3 rounded text-2xs font-semibold text-slate-400 hover:text-slate-100 ${focusRing}`}
            >
      ...
      {/* Data room */}
      <div className="panel p-5">
        <h3 className="text-sm font-semibold text-ivory-50">Required data room materials</h3>
        <p className="mt-1.5 text-2xs text-slate-500">
        ...
        <ul className="mt-3.5 grid gap-x-6 gap-y-2 sm:grid-cols-2 lg:grid-cols-3">
          {DATA_ROOM_MATERIALS.map((m) => (
            <li key={m} className="flex gap-2 text-2xs text-slate-400">
              <span
                aria-hidden="true"
      ...
      <div className="panel p-5">
        <h3 className="text-sm font-semibold text-ivory-50">What would change these ratings</h3>
        <p className="mt-1.5 text-2xs text-slate-500">
          Derived from which evidence gates are currently unmet, so the conditions shown are the
          ones that actually bind.
        </p>
        <dl className="mt-3.5 divide-y divide-white/[0.06]">
          {conditions.map((c) => (
            <div
              key={c.target}
            ...
            >
              <dt>
                <p className="text-xs font-medium text-slate-200">{c.target}</p>
                <p className="num mt-0.5 text-2xs text-slate-500">Currently {c.current}</p>
              </dt>
              <dd className="text-xs leading-relaxed text-slate-400">{c.condition}</dd>
            </div>
          ))}
        </dl>
      </div>
